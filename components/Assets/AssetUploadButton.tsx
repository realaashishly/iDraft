    ...
    (acceptedFiles: File[], fileRejections: FileRejection[]) => {
      ...
      if (fileRejections.length > 0) {
        ...
        if (rejection.file.size > MAX_SIZE) {
          setError(`File too large. Max size is ${formatBytes(MAX_SIZE)}.`);
        } else {
      ...
      >
        <DialogContent className="max-h-[80vh] overflow-y-auto sm:max-w-[500px]">
          ...
          <div className="space-y-4 py-4">
            {file ? (
              <div className="relative rounded-lg border border-dashed bg-background/50 p-4">
              ...
              <div
                {...getRootProps()}
                className={`relative w-full cursor-pointer rounded-lg border-2 border-dashed p-8 transition-colors ${
                  isDragActive
